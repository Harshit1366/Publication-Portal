 ...
 */
@WebServlet("/EditTech_Report")
public class EditTech_Report extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ps1 = conn.prepareStatement(
					"insert into tech_report(authors_name,dept,title,year,month_pub,date,remarks,tech_report,plag_report,username,revision) "
					+ "values (?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (!conn.isClosed()) {
					...
					while (itr.hasNext()) {
						...
						if (item.isFormField()) {
							...
							if ((itemname == null) || itemname.equals("")) {
								ResultSet rs1 = null;
								ps3 = conn.prepareStatement("select tech_report,plag_report from tech_report where id=?");
								ps3.setInt(1, id);
				...
				ResultSet rs = null;
				ps = conn.prepareStatement("select revision from tech_report where id=?");
				ps.setInt(1, id);
				rs = ps.executeQuery();
				...
				// int nrev=rev+1;
				//if (rev < 2) {
					ps2 = conn.prepareStatement("update tech_report set revision =? where id=?");
					ps2.setInt(1, (rev + 2));
				...
				ps4.setString(1, "Technical Report publication is edited by "+n+" for "+(rev+1)+" time/s");
