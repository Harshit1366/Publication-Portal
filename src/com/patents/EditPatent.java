 ...
 */
@WebServlet("/EditPatent")
public class EditPatent extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ps1 = conn.prepareStatement(
					"insert into patents(authors_name,dept,title,scope,country,app_no,app_year,app_date,award_year,award_date,patent_no,patent_file,plag_report,username,revision) "
					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (!conn.isClosed()) {
					...
					while (itr.hasNext()) {
						...
						if (item.isFormField()) {
							...
							if ((itemname == null) || itemname.equals("")) {
								ResultSet rs1 = null;
								ps3 = conn.prepareStatement("select patent_file,plag_report from patents where id=?");
								ps3.setInt(1, id);
				...
				ResultSet rs = null;
				ps = conn.prepareStatement("select revision from patents where id=?");
				ps.setInt(1, id);
				rs = ps.executeQuery();
				...
				// int nrev=rev+1;
				//if (rev < 2) {
					ps2 = conn.prepareStatement("update patents set revision =? where id=?");
					ps2.setInt(1, (rev + 2));
				...
				ps4.setString(1, "Patent publication is edited by "+n+" for "+(rev+1)+" time/s");
