package com.conf_presentations;
 ...
 */
@WebServlet("/EditConfPre")
public class EditConfPre extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ps1 = conn.prepareStatement(
					"insert into conf_presentations(authors_name,dept,title,presentation,scope,organisers,venue,year,month_pub,dates,hyperlink,pre_file,plag_report,username,revision) "
					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (!conn.isClosed()) {
					...
					while (itr.hasNext()) {
						...
						if (item.isFormField()) {
							...
							if ((itemname == null) || itemname.equals("")) {
								ResultSet rs1 = null;
								ps3 = conn.prepareStatement("select pre_file,plag_report from conf_presentations where id=?");
								ps3.setInt(1, id);
								...
								ps1.setString(12, file);
								ps1.setString(13, repo);
				...
				ResultSet rs = null;
				ps = conn.prepareStatement("select revision from conf_presentations where id=?");
				ps.setInt(1, id);
				rs = ps.executeQuery();
				...
				// int nrev=rev+1;
				//if (rev < 2) {
					ps2 = conn.prepareStatement("update conf_presentations set revision =? where id=?");
					ps2.setInt(1, (rev + 2));
				...
				ps4.setString(1, "Conference Presentation is edited by "+n+" for "+(rev+1)+" time/s");
