	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ps1 = conn.prepareStatement(
					"insert into book_chap(authors_name,dept,chap_no,chap_title,book_title,publisher,scope,year,month_pub,pages,book_isbn_no,hyperlink,pwsgte,link_index,chap_file,plag_report,username,revision) "
					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (!conn.isClosed()) {
					...
					while (itr.hasNext()) {
						...
						if (item.isFormField()) {
							...
							if ((itemname == null) || itemname.equals("")) {
								ResultSet rs1 = null;
								ps3 = conn.prepareStatement("select book_file,plag_report from books where id=?");
								ps3.setInt(1, id);
				...
				ResultSet rs = null;
				ps = conn.prepareStatement("select revision from book_chap where id=?");
				ps.setInt(1, id);
				rs = ps.executeQuery();
				...
				// int nrev=rev+1;
				//if (rev < 2) {
					ps2 = conn.prepareStatement("update book_chap set revision =? where id=?");
					ps2.setInt(1, (rev + 2));
				...
				ps4.setString(1, "Book Chapter publication is edited by "+n+" for "+(rev+1)+" time/s");
