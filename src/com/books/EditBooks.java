	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ps1 = conn.prepareStatement(
					"insert into books(authors_name,dept,book_title,publisher,scope,year,month_pub,pages,isbn_no,hyperlink,pwsgte,link_index,book_file,plag_report,username,revision) "
					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (!conn.isClosed()) {
					...
					while (itr.hasNext()) {
						...
						if (item.isFormField()) {
							...
							if ((itemname == null) || itemname.equals("")) {
								...
								ps1.setString(13, file);
								ps1.setString(14, repo);
				...
				ResultSet rs = null;
				ps = conn.prepareStatement("select revision from books where id=?");
				ps.setInt(1, id);
				rs = ps.executeQuery();
				...
				// int nrev=rev+1;
				//if (rev < 2) {
					ps2 = conn.prepareStatement("update books set revision =? where id=?");
					ps2.setInt(1, (rev + 2));
				...
				ps4.setString(1, "Book publication is edited by "+n+" for "+(rev+1)+" time/s");
